...
def searchForRange2(array, target):
    finalRange = [-1, -1]
    alteredBinarySearch2(array, target, 0, len(array) - 1, finalRange, True)
    alteredbinarySearch2(array, target, 0, len(array) - 1, finalRange, False)
    return finalRange

def alteredBinarySearch2(array, target, left, right, finalRange, goLeft):
    while left <= right:
        mid = (left + right) // 2
        if array[mid] < target:
            left = mid + 1
